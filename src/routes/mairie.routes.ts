...
const validateDeclarationId = async (req: Request, res: Response, next: Function) => {
  ...
  try {
    ...
    (req as any).declaration = declaration;
    next();
  } catch (error) {
 ...
 */
router.post('/declarations/:id/verify', 
  requireAuth, 
  requireRole(['mairie']),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const declaration = (req as any).declaration;
 ...
 *   get:
 *     summary: Lister les déclarations en attente de validation
 *     description: Récupère la liste des déclarations avec le statut 'pending' pour validation par la mairie
 *     tags: [Mairie]
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 *         description: Liste des déclarations en attente
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
router.get('/declarations/pending', requireAuth, requireRole(['mairie']), async (_req: Request, res: Response) => {
  try {
    const declarations = await Declaration.find({ status: 'pending' })
      .sort({ createdAt: -1 })
      .populate<{ parent: IParentInfo }>('parent', 'name email phone');
 ...
 *         description: Déclaration non trouvée
 */
router.post('/declarations/:id/validate', requireAuth, requireRole(['mairie']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 *         description: Déclaration non trouvée
 */
router.post('/declarations/:id/send-to-hospital', requireAuth, requireRole(['mairie']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
