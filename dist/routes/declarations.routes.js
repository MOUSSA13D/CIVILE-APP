 ...
 */
router.post('/', requireAuth, requireRole(['parent']), requireVerified, parseFormData, async (req, res, next) => {
    try {
        ...
        if (files?.certificatNaissance?.[0]?.path)
            uploadedFiles.push(files.certificatNaissance[0].path);
        if (files?.pieceIdentitePere?.[0]?.path)
            uploadedFiles.push(files.pieceIdentitePere[0].path);
        if (files?.pieceIdentiteMere?.[0]?.path)
            uploadedFiles.push(files.pieceIdentiteMere[0].path);
        if (files?.livretFamille?.[0]?.path)
            uploadedFiles.push(files.livretFamille[0].path);
        if (files?.justificatifDomicile?.[0]?.path)
            uploadedFiles.push(files.justificatifDomicile[0].path);
        uploadedFiles.forEach(filePath => {
            try {
                if (filePath && fs.existsSync(filePath)) {
                    fs.unlinkSync(filePath);
 ...
 */
router.get('/mairie/declarations/pending', requireAuth, requireRole(['mairie']), async (_req, res) => {
 ...
 */
router.post('/mairie/declarations/:id/verify', requireAuth, requireRole(['mairie']), async (req, res) => {
    const { id } = req.params;
 ...
 */
router.get('/hopital/declarations', requireAuth, requireRole(['hopital']), async (_req, res) => {
 ...
 */
router.post('/hopital/declarations/:id/response', requireAuth, requireRole(['hopital']), async (req, res) => {
    const { id } = req.params;
...
//# sourceMappingURL=declarations.routes.js.map